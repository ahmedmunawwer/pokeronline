...
const HANDS=[
    {name:"High Card",       hint:"The single highest card in your hand",         detail:"If no one has any combination, the player with the single highest card wins. Ace is highest (A > K > Q > J > 10 > ... > 2).",                                                                                            bg:"rgba(245,245,245,0.10)",border:"rgba(245,245,245,0.40)",clr:"rgba(240,240,240,0.95)",dim:"rgba(200,200,200,0.70)"},
    {name:"Pair",            hint:"Two cards of the same rank",                   detail:"Two cards of the same rank, regardless of suit. Higher pair beats lower pair — Aces beat Kings.",                                                                                                                           bg:"rgba(252,244,218,0.11)",border:"rgba(252,240,190,0.42)",clr:"rgba(252,244,218,0.95)",dim:"rgba(220,210,170,0.70)"},
    {name:"Two Pair",        hint:"Two different pairs",                          detail:"Two different pairs in one hand. Compared by the higher pair first; if tied, then the lower pair; if still tied, by the kicker (the 5th card).",                                                                             bg:"rgba(252,236,168,0.12)",border:"rgba(252,228,138,0.45)",clr:"rgba(252,238,190,0.95)",dim:"rgba(230,216,150,0.70)"},
    {name:"Three of a Kind", hint:"Three cards of the same rank",                 detail:"Also called 'trips' or 'set'. Higher three of a kind beats lower.",                                                                                                                                                          bg:"rgba(248,220,100,0.13)",border:"rgba(248,210,78,0.48)", clr:"#f0c040",              dim:"rgba(220,185,60,0.80)"},
    {name:"Straight",        hint:"Five cards in sequence",                       detail:"Five cards in numerical sequence, suits don't matter. Ace can be high (10-J-Q-K-A) or low (A-2-3-4-5). No wrap-around — Q-K-A-2-3 is NOT a straight.",                                                                     bg:"rgba(244,204,72,0.14)", border:"rgba(244,196,58,0.50)",clr:"#f0c040",              dim:"rgba(210,175,50,0.80)"},
    {name:"Flush",           hint:"Five cards of the same suit",                  detail:"Five cards of the same suit, any order. Higher top card wins ties.",                                                                                                                                                          bg:"rgba(240,192,64,0.15)", border:"rgba(240,192,64,0.52)",clr:"#f0c040",              dim:"rgba(200,165,45,0.80)"},
    {name:"Full House",      hint:"Three of a Kind + a Pair",                     detail:"Compared by the three of a kind first — Three Aces + Two Kings beats Three Kings + Two Aces.",                                                                                                                               bg:"rgba(230,160,50,0.16)", border:"rgba(228,152,38,0.54)",clr:"#e8a830",              dim:"rgba(195,135,35,0.80)"},
    {name:"Four of a Kind",  hint:"Four cards of the same rank",                  detail:"Also called 'quads'. Very rare. Higher four of a kind wins.",                                                                                                                                                                bg:"rgba(220,128,38,0.17)", border:"rgba(218,118,28,0.56)",clr:"#e09028",              dim:"rgba(180,115,28,0.80)"},
    {name:"Straight Flush",  hint:"Five cards in sequence, same suit",            detail:"Beats everything except a Royal Flush.",                                                                                                                                                                                      bg:"rgba(160,80,220,0.14)", border:"rgba(175,88,228,0.48)",clr:"#c8a0ff",              dim:"rgba(160,120,220,0.75)"},
    {name:"Royal Flush",     hint:"10-J-Q-K-A all same suit",                     detail:"The unbeatable hand.",                                                                                                                                                                                                       bg:"rgba(140,60,200,0.18)", border:"rgba(160,80,220,0.55)",clr:"#ddb8ff",              dim:"rgba(180,140,230,0.75)"},
];
            ...
            <div style={{maxWidth:400,width:"100%",maxHeight:"86vh",overflowY:"auto",background:"rgba(8,18,12,0.98)",border:"1px solid rgba(240,192,64,0.3)",borderRadius:20,padding:"18px 14px",boxSizing:"border-box"}}>
                ...
                {HANDS.map((hr,i)=>(
                    <div key={i} onClick={()=>toggle(i)} style={{background:hr.bg,border:`1px solid ${hr.border}`,borderRadius:10,padding:"10px 12px",marginBottom:7,cursor:"pointer",userSelect:"none"}}>
                        ...
                        {open[i]&&<div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${hr.border}`,color:hr.dim,fontSize:13,lineHeight:1.55}}>{hr.detail}</div>}
                    </div>
                ))}
            </div>
        </div>
